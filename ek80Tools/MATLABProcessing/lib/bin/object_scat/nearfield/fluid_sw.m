% scattering by a fluid sphere due to a spherical wave
% pressure field
...
if ( proc_flag == 1)
   ...
   sign_term=(-1).^n;
   for j=1:m
      s=nl.*pn.*bn(j,:).*hn_r(j,:).*hn_rho(j,:).*sign_term;
      s1=-i*2*nl.*pn.*bn(j,:)/ka1(j);
      p(j)=i*krho(j)*sum(s);
      p1(j)=exp(i*(krho(j)+kr(j)))*sum(s1)/(2*r_to_a);
%      [amp, engy_r(j)]=energy(j,s,ka1(j)); engy(:,j)=amp(:);
  ...
  xlabel('ka')
   ...
   bn=1./(1+i*cn);
   sign_term=(-1).^n;
   rest=nl.*bn.*hn_r.*hn_rho.*sign_term/ka1;
   for j=1:m
      s=(pn(j,:)).*rest;
      f(j)=abs(sum(s));
      fc(j)=sum(s);
