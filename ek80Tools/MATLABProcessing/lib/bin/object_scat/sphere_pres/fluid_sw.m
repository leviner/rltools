% scattering pressure field by a fluid sphere due to a spherical wave 
% function	[var,ps]=fluid_sw(proc_flag,scale,out_flag,para)
...
%  para(8) = Ra 	:  R/a  - R: distance from the source to the scatterer
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  Dezhang Chu,			7-13-99
%
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ( proc_flag == 1)
   ...
   sign_term=(-1).^n;
   for j=1:m
      s=nl.*pn.*bn(j,:).*hn_r(j,:).*hn_R(j,:).*sign_term;
      pc(j)=-i*kR(j)*sum(s);
   end
   var=ka1;
else
   ...
   bn=1./(1+i*cn);
   sign_term=(-1).^n;
   rest=nl.*bn.*hn_r.*hn_R.*sign_term;
   for j=1:m
      s=(pn(j,:)).*rest;
      fc(j)=sum(s);
   end
...
%% rho = dist. from the source to the receiver
rho_to_a=sqrt(Ra^2+ra^2+2*Ra*ra*cos(theta));
pinc=exp(i*ka1.*rho_to_a);
switch out_flag
  case 1
    ps=abs(pc);
  case 2
